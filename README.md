...
| **Target distribution (v0.15.0)** | **works - unit-tested** | `src/fleet/claims.mjs`: trip claims stop the measured "19 bots -> one beach" pile-up (run 35485296464: banked=0 smelted=0 sand=0 with sand=110 KNOWN positions, 38x 'map trip skipped: unreachable'). When a bot commits to a map target it registers a TTL claim (120 s, death-safe); every other bot scores candidates as distance + 96 penalty when a foreign claim sits within 32 blocks - a radius, not a point, because a beach is 100+ positions of one cluster. Soft: a fully-claimed fleet still picks the best option instead of idling. In-process boards are shared like the WorldMap; cross-process claims travel as one-line `PVB2\|claim\|<owner>\|<x>,<y>,<z>` chat messages (the PVB1 counterpart, ignored by old parsers). Wired into `mapTargetFor` (via `WorldMap.nearestK`, k candidates instead of 1), `mapTrip`, `workOnGround` map trips; fleet19 reports `claims=` and `claimedHolds=` |
...
| **Wet-escape traverse (v0.17.0)** | **works - unit-tested + e2e PASS** | the flooded-shaft trap, closed: the 17:05 fleet measured F7 spending its WHOLE 450s run at y=55 - climbOut refused every rotation with dug=0 (the cells above its head were water: the shaft had become a 1x1 well) while the drown rescue timed out 'still wet' 4x (a down-flow beats swim-up; no shore inside the scan radius). climbOut now detects the wet refusal, digs a guarded horizontal gallery out from under the water (`traverseStep`: waterfall guard above, gap/wet floor guard below, waterlogged cells refuse) with patient digs (`fastDig maxTicks 200` - a submerged miner digs 5x slower, no aqua affinity) and staircases up from the dry gallery; the drown sentry yields while the escape owns the controls (bounded 2 galleries x 20s). e2e (testbed/diag-climb-wet.mjs): bot dropped 8-deep into a flooded well - walked 9+10 blocks of galleries, breached the surface, ok=true, hp 20 |
| **Trip walk budget (v0.17.0)** | **works - unit-tested** | `src/lib/tripplan.mjs`: the flat 14s map-trip timeout killed F4's legitimate 100-block trips ('unreachable' AFTER the climb was already paid, claim registered, TTL steering everyone away). The budget now scales with distance (250 ms/block at pathfinder ground speed ~4 b/s, 14s base) and caps at exactly the trip radius the map promises (128 blocks at 4 b/s = 32s) - the v0.11.2 A*-expansion OOM lesson stays honoured |
