...
- RUN100 MINED (35874523075, SUCCESS): NORMAL END, 19/19 (F6 zombie death mid-run, respawned), mined 3831 @ 6.38 b/s, banked=1469 (from 586), smelted=15 (from 1), conversion 75.7%, fights=15 (from 33), airGlitches=52 (from 835), torched=8, plan 2/31. THE CLOCK CAP HELD: F4's full honest window (fuel clips 3 x oak_log -> clock clips 81s/7 of 18 -> smelting 4 x raw_copper -> took 4 x copper_ingot) - the first funded metal smelt of the new era; junk lane landed too (F8/F10 stone). Tithe fired 3x, the junk coal floor held (pocket keeps 6).
- THE RUN100 DISEASE - THE CHEST DOOM WALL: the yard chest row [-113..-143,70,398-408] doom-ledgered by ONE bot's far-start 'No path' (F8 from d=46, F16 from d=17!), then refused the fuel commons/hops/final banks at 'ledgered 16s/19s/25s ago' (refusals land 16-25s AFTER the recording failure) - the 90s default poison outlives every bank chain (56-88s staggered). Failure classes: timeouts 50, No path 36, doomed refusals 33, Took-to-long 3. F17's raw_iron died to 'no machine in reach 48b' - the mobility wall IS the iron wall.
- SHIPPED THE CHEST DOOM HALF-LIFE (v0.113.0, f64b493): CHEST_DOOM_TTL_MS=15000 in deposit.mjs (the v0.92.0 machine semantics for chests - static known-good destination, the doom is the WALK's sickness). Three walkers pass it (deposit walkOnce, fuel commons, fleet19 yard walk) + the noPathLedger records BOTH shapes at 15s (one truth about a cell). The v0.96.0 absorption keeps the first failure's clock (storms cannot immortalize). +3 test blocks; deposit-walk 22/22, unit 76/76 files, check-syntax 177/0. .gitignore += run75/.

Stage Summary:
- Master: v0.113.0 (f64b493). Next free version = 0.114.0.
- EXPECTATIONS run101: 'ledgered 16-25s ago' refusals on chest walks shrink to the fresh window (<15s); the fuel commons re-admits sick chests after 15s; final banks stop exhausting on poisoned rows (F13-class 173u stranding shrinks); unaccounted shrinks toward the pocket-read floor; the iron ladder gets its mobility (raw_iron reaches machines); the clock-cap/floor/tithe cures hold.
- OPEN FRONTS: (a) iron=0 pickaxes (now mobility-bound); (b) F6-class open-field zombie death (shelter skip: no diggable wall, ring incomplete); (c) unaccounted=931; (d) plan 2/31; (e) worldmap idle; (f) the doomed BAY (machines) still 15s - the deep bay [-138,43,441] refused at 9s (correctly, fresh); (g) 22s walk timeouts at d=18-25 (the walk sickness itself).
- The fleet dispatch fires as the ABSOLUTE LAST action of the session (run id to be confirmed by the next session from the runs API).
...
Work Log:
- run101 = 35881462426 - discovered ALREADY DISPATCHED on ff2f497 at 15:25:36Z by the parallel lane (the 22:05 session's protocol ending). NOT re-dispatched (one fleet per head); no pushes until it completed.
- Push-CI on ff2f497 = SUCCESS (unit x2 + integration). The fleet FAILED at ts~241s: the run53/run92 OOM class - stormguard STORM PROBE (rss 629->1668M, +208MB/s, mainLate 2127ms) then FATAL second strike (1668->2537M), exit 143 (the designed readable amputation). pf:goal/done 'walk' cycle ~1.1s.
- THE DECODE - THE QUEUE WALL: the pathfinder queue 6a/8-12q from ts=141 (80s+), mined FROZEN at 92, mainLate 1.9-3.0s, rss HEALTHY 356-360M until the burst, ZERO [allocvalve] lines (the 600M floor armed inside the burst; the >24b cut misses the short-walk churn). run100 (SUCCESS) peaked 10-11q ONCE each - the difference is PERSISTENCE.
- SHIPPED THE QUEUE-PRESSURE ARM (v0.115.0, f5f7052): (a) queued >= 10 on 30 consecutive 1s ticks closes the valve (source 'queue-pressure', the storm refusal semantics + escalation; run101's wall closes ~50s before the burst, run100's isolated bursts reset before 30); (b) the floor 600 -> 450M; (c) the ticker feeds the semaphore's queue depth via startFleetValveTicker (no circular import); (d) the refusal/CLOSED/FLEET-RESULT lines name the pressure; (e) null/NaN readings HOLD the streak (Number(null)=0 never masquerades as '0 queued'). +6 test blocks; allocvalve 38/38, unit 76/76 files, check-syntax 177/0.

Stage Summary:
- Master: v0.115.0 (f5f7052). Next free version = 0.116.0.
- EXPECTATIONS run102: a sustained 10q+ wall now closes the valve EARLY ('[allocvalve] CLOSED: path queue Nq sustained' lines, queueCloses > 0 in the FLEET RESULT) - NO second-strike SIGTERM; the rss floor 450M arms the burst verdict sooner if the storm comes anyway; NORMAL END resumes; the v0.114.0 cures measured (the poison lens verdicts + the chest doom half-life refusals).
- OPEN FRONTS: (a) the short-walk churn under saturation (the valve cannot cut it - rescues must flow; the stormguard stays the amputation); (b) iron=0 pickaxes (mobility-bound); (c) the witch front (the parallel lens unmeasured); (d) plan 2/31; (e) worldmap idle.
- The fleet dispatch fires as the ABSOLUTE LAST action of the session (run id to be confirmed by the next session from the runs API).
...
Agent: Z.ai Code (cron session, 00:53 +08)
Task: mine run102 (the v0.116.0 fleet); ship the cure the evidence names; push; dispatch.

Work Log:
- Inherited briefing STALE (v0.108.0/run97); worklog+git truth: master v0.116.0, run102 = 35889087936 in flight -> polled to SUCCESS.
- RUN102 MINED (run36/): NORMAL END, 19/19, mined 3268 @ 5.45, banked 2366, smelted 28, unaccounted=0, conversion 100.1%. QUEUE-PRESSURE ARM CONFIRMED (closed ts=72s 'path queue 13q sustained', no FATAL). Doom half-life healthy (all refusals fresh <15s). Witches never engaged. THE DISEASE: the chronic-liar bar - F3 151+ glitch pages, 11 overrides, 15 rescues, 10 dry-land proofs, 4 frozen relogs (the 20s gate kept counting the stale streak -> a no-op rescue every ~25s).
- SHIPPED THE CHRONIC-LIAR LADDER (v0.117.0, b881a09): gate holds the streak; the glitch-class proof ratchets the override bar 8/16/24/32/40 bounded; resets on wet contact and non-proof exits; junk-safe dryGlitchCap in waterVerdict. +5 test blocks; drowning 77/77, unit 76/76 files, check-syntax 177/0. .gitignore += run36/ + leftovers.

Stage Summary:
- Master: v0.117.0 (b881a09). Next free version = 0.118.0.
- EXPECTATIONS run103: liar-ladder ratchet lines, overrides 11 -> ladder count, frozen relogs 4 -> fewer, rescue churn down, NORMAL END holds, arm keeps early closes.
- OPEN FRONTS: fall/env x3 deaths (new top class), iron pickaxe craft walk, tool repair lane, plan 1/31, worldmap idle.
- Dispatch run103 = the ABSOLUTE LAST action (record line pushes after in_progress).

---
Task ID: 398294-20260924-0154
Agent: Z.ai Code (cron session, 01:54 +08)
Task: mine run103 (the v0.117.0 fleet); ship the cure; push; dispatch.

Work Log:
- Sandbox died (3rd rebuild): fresh clone + npm install. Master moved to v0.118.0 (the lane's parseDeathMessage, Collision #43). run103 = 35895546754 SUCCESS -> mined run54/.
- DECODE: NORMAL END, 19/19, 3478 @ 5.80 b/s, valve 0 closes (no storm). Liar ladder held (F17 ratcheted 10x to the 40 bound). THE DISEASE: the frozen-relog LOOP - 30 wet-frozen relogs, F14 12x into the same column [-121,58-59,376] (page o2 12-13 -> surface o2 20 -> physics freeze -> relog -> same column -> re-page).
- SHIPPED THE FROZEN-RETURN GATE (v0.119.0, c0ce208): per-bot process-wide streak Maps; the sentry holds non-critical pages 10/20/40/60s bounded per consecutive relog; honest completion clears; critical bar (o2<=4) bypasses. +4 test blocks; drowning 81/81, unit 77/77, check-syntax 179/0.

Stage Summary:
- Master: v0.119.0. Next free = 0.120.0. EXPECTATIONS run104: relog churn 12->2-4 per cycler, named holds/clears, relogins 40-><15, NORMAL END holds. FRONTS: iron pickaxe craft walk, the 40-bound ceiling question, conversion 85.2%, worldmap iron targets unconsumed.
- Dispatch = the ABSOLUTE LAST action (record pushes after in_progress).
...
Work Log:
- run104 (35899827086, v0.118.0) = SUCCESS; push-CI f1927e0 (v0.119.0) = SUCCESS; run105 = 35903689995 dispatched as the LAST action, in_progress on f1927e0; dispatch record pushed (43b15a2). Next free version = 0.120.0. Next session: mine run104 (the death-cause truth) + run105 (the frozen-return gate).

---
Task ID: 398294-20260924-0254
Agent: Z.ai Code (cron session, 02:54 +08)
Task: mine run105 (the first fleet of v0.119.0); fix the red it named; push; keep CI green.

Work Log:
- run105 = 35903689995 (CI run 498) FAILED: the fleet job died of the run92/run53 OOM class (stormguard FATAL exit 143 at ts~421s) with the alloc valve NEVER closing - zero [allocvalve] lines, third time (run92/93/105). Anatomy: rss 446-449M (under the 450M floor) -> 635M over ~14s (13MB/s, sub-threshold) -> 1750M in one 5s window (223MB/s) -> 2604M, stack all wood-trip/furnace-walk A* cycles (the aggregate long-A* shape; queue depth never built, the queue-pressure arm blind to it by design). THE STRUCTURAL GAP: the worker published the first-strike verdict into the storm cell and it DIED THERE - both valve feeders (the 1s ticker + the cell poll inside it) live on the starved TIMER phase, while the funnel's pf notes marched through the kill window at 0.2s cadence.
- SHIPPED THE FUNNEL PROBE (v0.121.0, ab8cd42): every gotoSafe consult polls the storm cell and applies a fresh worker verdict INLINE + reads rss on the consult path and closes on the funnel's own verdict (floor 450M, bar 80MB/s = 2x the worker's, the FULL gain over a real 150ms+ gap - GC noise never clears it). Named '[allocvalve] CLOSED (funnel probe)' lines in two flavors; FLEET RESULT counts funnelCloses+funnelCellCloses; junk-safe throughout (throwing reader judges nothing; sub-gap spike keeps the anchor; dip resets; stale seq never re-applied).
- +10 test blocks; allocvalve 43/43, goto-safe 32/32, full unit 77/77 files locally (pure node --test only), check-syntax 179/0. Pushed ab8cd42 = v0.121.0; push-CI run 35907649186. Repo worklog updated in the same shape.

Stage Summary:
- Master: v0.121.0 (ab8cd42). Next free version = 0.122.0.
- EXPECTATIONS (next fleet): a run105-class storm costs a 12-30s walk outage with '(funnel probe)' lines and the run SURVIVES; NORMAL END returns. The suffocation cure (run104's F18 proof) is now the top front, then the witch ceiling, iron pickaxe craft walk, conversion, plan progress, worldmap iron consumption.
- run105 died before the frozen-return gate's holds could matter; the gate rides the next fleet again.
